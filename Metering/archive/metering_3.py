...
class ATM90E3x:
    ...
    def read_voltage(self):
        """Read RMS Voltage."""
        voltage = self._read_register(0x0001) * 0.01
        logging.info("Voltage: %.2f V", voltage)
        return voltage

    def read_current(self):
        """Read RMS Current."""
        current = self._read_register(0x0002) * 0.001
        logging.info("Current: %.3f A", current)
        return current

    def read_power(self):
        """Read Active Power."""
        power = self._read_register(0x0003) * 0.01
        logging.info("Power: %.2f W", power)
        return power

    def read_energy(self):
        """Read Total Energy."""
        energy = self._read_register(0x0004) * 0.001
    ...
    def read_power_factor(self):
        """Read Power Factor."""
        power_factor = self._read_register(0x0006) * 0.001
        logging.info("Power Factor: %.3f", power_factor)
        return power_factor

    def read_phase_angle(self):
        """Read Phase Angle."""
        phase_angle = self._read_register(0x0007) * 0.01
    ...
    def get_status_flags(self):
        """Read Status Flags."""
        status = self._read_register(0x0200)
