...
class ATM90E3x:
    ...
    def init_gpio(self):
        ...
        # Setup pins as output or input depending on the function
        GPIO.setup([PINS['ZX0'], PINS['ZX1'], PINS['ZX2'], PINS['CF1'], PINS['CF2'], PINS['CF3'],PINS['CF4'], PINS['WarnOut'], PINS['IRQ0'], PINS['IRQ1'], PINS['PM0'], PINS['PM1'],PINS['DMA'], PINS['RST']], GPIO.OUT)
    
        
        # Setup input pins for event detection (e.g., Zero-crossing, interrupt signals)
        GPIO.setup([PINS['ZX0'], PINS['ZX1'], PINS['ZX2'], PINS['WarnOut'], PINS['IRQ0'], PINS['IRQ1']], GPIO.IN)
    
    # Zero-crossing event handler
    def on_zero_crossing(self,channel):
        print(f"Zero Crossing Detected on Pin {channel}")
    
    # Interrupt request event handler
    def on_interrupt_request(self,channel):
        print(f"Interrupt Request Detected on Pin {channel}")
