...
class FanController:
    ...
    def calculate_target_rpm(self, fan_name):
        ...
        min_rpm = self.fan_status[fan_name]['min_rpm']
        max_rpm = self.fan_status[fan_name]['max_rpm']
        pwm_value = self.fan_status[fan_name]['speed']

        # Scale the target RPM proportionally based on PWM
        target_rpm = min_rpm + ((max_rpm - min_rpm) * (pwm_value / 255))
        logging.info(f"Target RPM for {fan_name} based on current PWM {pwm_value}: {target_rpm}")
        return target_rpm
    ...
    def monitor_fans(self):
        ...
        self.check_drive_fail_status()
        for fan_name in self.fan_settings:
