...
class FanController:
    ...
    def control_fans_based_on_temperature(self, temperature_data):
        ...
        for fan_name, temp in temperature_data.items():
            ...
            if temp < self.DEFAULT_TEMP_THRESHOLDS[0]:
                self.set_fan_speed(fan_name, 20)  # Low speed
            elif temp < self.DEFAULT_TEMP_THRESHOLDS[1]:
                self.set_fan_speed(fan_name, 50)  # Medium speed
            else:
                self.set_fan_speed(fan_name, 100)  # High speed
