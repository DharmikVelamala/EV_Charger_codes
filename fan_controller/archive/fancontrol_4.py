...
class FanController:
    ...
    def __init__(self, i2c_bus):
        ...
        self.fan_status = {
            name: {'speed': 0, 'error': False, 'rpm': 0, 'target_rpm': 0} for name in self.FAN_ADDRESSES
        }
    ...
    def compare_rpm_with_target(self, fan_name, tolerance=0.10):
        """
        Compare the fan's RPM with the dynamically calculated target RPM.

        :param fan_name: Name of the fan (e.g., "fan1").
        :param tolerance: Tolerance percentage (default is 10%).
        """
        if fan_name not in self.FAN_ADDRESSES:
            logging.error(f"Invalid fan name: {fan_name}")
            return

        actual_rpm = self.fan_status[fan_name]['rpm']
        target_rpm = self.fan_status[fan_name]['target_rpm']

        if target_rpm == 0:
            logging.warning(f"Target RPM for {fan_name} is not calculated.")
            return

        lower_bound = target_rpm * (1 - tolerance)
        upper_bound = target_rpm * (1 + tolerance)

        if not (lower_bound <= actual_rpm <= upper_bound):
            logging.error(f"{fan_name}: Actual RPM ({actual_rpm}) is outside the tolerance range. Target RPM: {target_rpm}, Lower Bound: {lower_bound}, Upper Bound: {upper_bound}")
        else:
            logging.info(f"{fan_name}: Actual RPM ({actual_rpm}) is within tolerance. Target RPM: {target_rpm}, Lower Bound: {lower_bound}, Upper Bound: {upper_bound}")
    ...
    def monitor_fans(self):
        ...
        for fan_name in self.FAN_ADDRESSES:
            ...
            if rpm is not None:
                ...
                self.compare_rpm_with_target(fan_name)
            else:
